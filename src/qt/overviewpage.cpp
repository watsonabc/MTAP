...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strAnonymizemtapAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizemtapAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizemtapAmount = strAnonymizemtapAmount.remove(strAnonymizemtapAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizemtapAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizemtapAmount * COIN) nMaxToAnonymize = nAnonymizemtapAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizemtapAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizemtapAmount));
        strAnonymizemtapAmount = strAnonymizemtapAmount.remove(strAnonymizemtapAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizemtapAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    } else {
