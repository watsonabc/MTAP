...
unsigned int nStakeMinAge = 60 * 60; // 1hr
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight >= 1 && nHeight <= 1500 ) {
	        nSubsidy = 1 * COIN;
	} 
	else if (nHeight <= 2000 && nHeight > 1500) {
        nSubsidy = 2 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < 1501) {
        ...
        ret = blockValue * 0.8; // 80% of block reward for
    }
...
static CBlockIndex* FindMostWorkChain()
{
    do {
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            // for the most work chain if we come mtaposs them; we can't switch
