    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat mtap and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 MTAP per transaction on average.
            */
            if (r <= 10) {
...
bool CPrivateSendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizemtapAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CPrivateSendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100MTAP+1 ( bit on if present )
    // bit 1 - 10MTAP+1
    // bit 2 - 1MTAP+1
    // bit 3 - .1MTAP+1
    // bit 3 - non-denom
