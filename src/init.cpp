...
// Copyright (c) 2018 The mtap developers
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 10155, 39795));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, mtap, (privatesend, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all mtap specific functionality (Masternodes, PrivateSend, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:10155"));
    ...
    strUsage += HelpMessageOpt("-anonymizemtapamount=<n>", strprintf(_("Keep N MTAP anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 10156, 39799));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018 The mtap Core Developers")) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizemtapAmount = GetArg("-anonymizemtapamount", 0);
    if (nAnonymizemtapAmount > 999999) nAnonymizemtapAmount = 999999;
    if (nAnonymizemtapAmount < 2) nAnonymizemtapAmount = 2;
    ...
    LogPrintf("Anonymize mtap Amount %d\n", nAnonymizemtapAmount);
    ...
    /* Denominations
       ...
       For example:
       1MTAP+1000 == (.1MTAP+100)*10
       10MTAP+10000 == (1MTAP+1000)*10
    */
