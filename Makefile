...
pkgdatadir = $(datadir)/MTAP
pkgincludedir = $(includedir)/MTAP
pkglibdir = $(libdir)/MTAP
pkglibexecdir = $(libexecdir)/MTAP
...
CONFIG_HEADER = $(top_builddir)/src/config/mtap-config.h
...
ACLOCAL = ${SHELL} /home/callum/MTAP/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/callum/MTAP/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/callum/MTAP/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/callum/MTAP/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /home/callum/MTAP/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = MTAP
PACKAGE_BUGREPORT = www.mtap.io
PACKAGE_NAME = MTAP Core
PACKAGE_STRING = MTAP Core 1.0.0
PACKAGE_TARNAME = MTAP
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/callum/MTAP
abs_srcdir = /home/callum/MTAP
abs_top_builddir = /home/callum/MTAP
abs_top_srcdir = /home/callum/MTAP
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/callum/MTAP/build-aux/install-sh
...
SUBDIRS = src
BITCOIND_BIN = $(top_builddir)/src/mtapd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/mtap-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/mtap-cli$(EXEEXT)
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_mtap_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_mtap_coverage.info test_mtap.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/MTAP-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/mtap-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/mtap-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/mtap-config.h
$(top_srcdir)/src/config/mtap-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/mtap-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/mtap-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/MTAP-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/MTAP-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_mtap.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_mtap -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_mtap_filtered.info: test_mtap.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_mtap_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_mtap_coverage.info: baseline_filtered_combined.info test_mtap_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_mtap_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_mtap_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_mtap_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_mtap.coverage/.dirstamp:  test_mtap_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_mtap.coverage/.dirstamp total.coverage/.dirstamp

# mtap: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_mtap.coverage/ total.coverage/ $(OSX_APP)
